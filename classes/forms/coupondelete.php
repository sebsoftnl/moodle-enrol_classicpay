...
/**
 * enrol_classicpay\forms\coupondelete
 *
 * @package     enrol_classicpay
 *
 * @copyright   Sebsoft.nl
...
class coupondelete extends \moodleform {
    ...
    public function definition() {
        ...
        if ($a->courseid > 0) {
            ...
            $a->course = get_string('entiresite', 'enrol_classicpay');
        }
        ...
        $mform->addElement('header', 'formheader', get_string('coupon:delete', 'enrol_classicpay'));
        $mform->addElement('static', 'formstatic', '', get_string('coupon:delete:warn', 'enrol_classicpay', $a));
        $mform->addElement('hidden', 'id');
        $mform->setType('id', PARAM_INT);
        $this->add_action_buttons(true, get_string('coupon:delete', 'enrol_classicpay'));
    }
     ...
     * of the form.
     *
     * @param int $id id from table enrol_classicpay_coupon
    ...
    public function process_post($redirect) {
        ...
        global $DB;
        $DB->delete_records('enrol_classicpay_coupon', array('id' => $data->id));
        $DB->delete_records('enrol_classicpay_cuse', array('couponid' => $data->id));
        redirect($redirect, get_string('coupon:deleted', 'enrol_classicpay'));
        exit;
    }
